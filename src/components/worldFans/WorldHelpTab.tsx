    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-black text-slate-800 dark:text-white flex items-center gap-2">
            <Compass size={20} className="text-emerald-600 dark:text-emerald-400" />
            <span>دليل ومساعدات المغتربين السكندريين</span>
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
            تجارب، نصائح، واستفسارات من الاتحاداوية المقيمين بالخارج لمساعدة القادمين الجدد
          </p>
        </div>
        ...
        >
          <Plus size={16} />
          <span>طرح استفسار أو مساعدة</span>
        </button>
      </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="text-center py-12 rounded-3xl bg-slate-50 dark:bg-slate-800/40 border border-dashed border-slate-200 dark:border-slate-700">
          <HelpCircle size={28} className="text-emerald-500 mx-auto mb-2" />
          <h4 className="text-sm font-black text-slate-800 dark:text-white mb-1">لا توجد استفسارات في هذا القسم</h4>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">اطرح استفسارك بخصوص السكن، العمل، أو مشاهدة المباريات في بلدك!</p>
        </div>
      ) : (
          ...
          {filteredRequests.map((req) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-3xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/70 p-4 sm:p-5 shadow-sm"
              >
                {/* Top Info */}
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2">
                    <img
                      src={req.userAvatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80'}
                      alt=""
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 font-semibold">
                        {req.countryName} {req.city ? `(${req.city})` : ''} • {new Date(req.createdAt).toLocaleDateString('ar-EG')}
                      </span>
                    </div>
                  </div>
                  ...
                  <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-full border ${
                    ...
                    {req.status === 'resolved' ? 'تمت الإجابة ✓' : 'مفتوح للنقاش'}
                  </span>
                </div>
                    ...
                    {req.replies && req.replies.length > 0 ? (
                      <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                        {req.replies.map((reply) => (
                          <div key={reply.id} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/60 text-xs">
            ...
            <form onSubmit={handleCreateRequest} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="match_streaming">مشاهدة مباريات وسيرفرات 📺</option>
                    <option value="legal_visas">إقامات وأوراق رسمية 📄</option>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="مثال: كيف أتابع مباريات الاتحاد في ألمانيا؟ / أماكن سكن قريبة من المترو بالرياض"
                  className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-semibold"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">تفاصيل الاستفسار *</label>
                <textarea
                  rows={4}
                  required
                  value={content}
                  onChange={(e) => setContent(e.target.value)}
                  placeholder="اشرح استفسارك بالتفصيل ليستطيع أبناء الرابطة إفادتك بأفضل النصائح..."
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                >
                  {isSubmitting ? 'جاري النشر...' : 'نشر الاستفسار'}
                </button>
              </div>
            </form>
          </motion.div>
